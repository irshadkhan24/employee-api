...
const getEmployees = async (req, res) => {
    try {
        const employees = await Employee.find().populate('userId', {password: 0}).populate("department")     // data extract and show in screen
...
const getEmployee = async (req, res) => {
    ...
    try {
        let employee;
        employee = await Employee.findById({_id: id}).populate('userId', {password: 0}).populate("department")     // data extract and show in screen
        if(!employee) {
...
const fetchEmployeesByDepId = async (req, res) => {
    const {id} = req.params;
    try {
        const employees = await Employee.find({department: id})    // data extract and show in screen
        return res.status(200).json({success: true, employees});
    } catch(error) {
