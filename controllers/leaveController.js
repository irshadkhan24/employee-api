...
const getLeaves = async (req, res) => {
    try {
        ...
        console.log(error.message)
        return res.status(500).json({success: false, error: "leave ad d server error"})
   }
...
const getLeaveDetail = async (req, res) => {
    try {
        const { id } = req.params;        //lowercase 'params'
        const leave = await Leave.findById({_id: id}).populate({
            ...
            populate: [
                ...
                {
                    path: 'userId',
                    select: 'name profileImage'      // No comma in select string
                }
            ]
        });
